...
public:
	...
	ScriptNodeId CreateNode(ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& node, Vector2f pos);
	...
	/// </summary>
	void CreateNodeWithReusedId(ScriptNodeId id, ScriptNodeTypeId typeId, std::unique_ptr<ScriptNodeBase>&& node, Vector2f pos);
	/// <summary>
	...
	Vector2f GetPosition(ScriptNodeId id) const;
	...
	void SetPosition(ScriptNodeId id, Vector2f pos);

...
#include "../Editor/Commands/CreateLinkCommand.h"
#include "../Editor/Commands/CreateNodeCommand.h"
#include "../Editor/Commands/DestroyNodeAndLinksCommand.h"
#include "../Editor/Commands/FixupSelectionCommand.h"
#include "../Editor/Commands/MoveNodesCommand.h"
#include "../Editor/Commands/SetOverridenValueCommand.h"
...
void ScriptEditor::SaveFile(EditorScriptData& anActiveScript, Script& aScript)
{
	// use GameAssetPath when saving to ensure we always save to the games data path
	std::string basePath = STATIC_ASSET_JSON_SCRIPT_PATH;

	std::filesystem::path scriptPath = std::filesystem::path(basePath) / std::filesystem::path(myActiveScript).replace_extension(".json");

	ScriptJson jsonData;
	aScript.WriteToJson(jsonData);

	std::ofstream out(scriptPath);
	out << jsonData.json;
	out.close();

	anActiveScript.latestSavedSequenceNumber = aScript.GetSequenceNumber();
}
...
void ScriptEditor::Init()
{
	...
	std::string path = STATIC_ASSET_JSON_SCRIPT_PATH;
...
void ScriptEditor::Update()
{
		...
		if (ImGui::BeginPopupModal("Create new script", NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			...
			{
				std::string sceneName = MainSingleton::GetInstance().GetSceneManager().GetActiveScene()->GetSceneName();
				sceneName += "-";
				sceneName += newScriptName;
				strncpy_s(newScriptName, sceneName.c_str(), sizeof(newScriptName));
		...
		if (!myActiveScript.empty())
		{
			...
			if (ImGui::Button("Delete"))
			{
				...
				for (const auto& entry : std::filesystem::directory_iterator(basePath))
				{
					...
					if (filename.find(myActiveScript) == 0)
					{
						myOpenScripts.erase(myActiveScript);
						std::filesystem::remove(entry.path());
						if (!myOpenScripts.empty())
							myActiveScript = myOpenScripts.begin()->first;
						else
							myActiveScript = "";
			...
			for (ScriptNodeId currentNodeId = script.GetFirstNodeId(); currentNodeId.id != ScriptNodeId::InvalidId; currentNodeId = script.GetNextNodeId(currentNodeId))
			{
				...
				for (int i = 0; i < inCount; i++)
				{
					...
					widthLeft = max(widthLeft, labelWidth);
				...
				for (int i = 0; i < outCount; i++)
				{
					...
					widthRight = max(widthRight, labelWidth);
				}
...

				{
					for (int i = 0; i < inCount; i++)
					{
						...
						if (connectionCount == 0)
						{
							...
							ImGui::PushItemWidth(max(20.f, widthLeft - labelWidth));

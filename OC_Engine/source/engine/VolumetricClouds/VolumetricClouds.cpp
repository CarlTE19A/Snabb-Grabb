...
void VolumetricClouds::Init()
{
	...
	myPhaseParams = { 0.5f, 0.5f, 0.023f, 0.03f };
	myCloudScale = 0.01f;
	myDensityThreshold = 0.557f;
	myDensityMultiplier = 0.313f;
	myNumSteps = 50;
	lightAbsorptionThroughCloud = 0.012f;
	numStepsLight = 4;
	darknessThreshold = 0.113f;
	detailWeight = 0.5f;
}

void VolumetricClouds::Update(const Vector3f aFollowPos)
{
//#ifdef _DEBUG
...
//#endif
	...
	Vector3f newPos = { aFollowPos.x, myTransform.GetPosition().y, aFollowPos.z };
...
void VolumetricClouds::Render()
{
	...
	myNoiseTexture.BindSRV(7, context);
	...
	context->PSSetShaderResources(7, 1, &null2);
...
bool VolumetricClouds::InitShadersAndBuffers()
{
	...
	auto device = GraphicsEngine::GetInstance()->GetDevice();
	{
		...
		if (FAILED(result))
		{
			LogError("Failed to create vertex buffer, volumetric clouds");
			return false;
		}
	}
	...
	myIndexCount = static_cast<unsigned int>(cubeIndices.size());
	{
		...
		if (FAILED(result))
		{
			LogError("Failed to create index buffer, volumetric clouds");
			return false;
		}
	}
	...
	//create cloud buffer
	{
		...
		if (FAILED(result))
		{
			LogError("failed to create cloudbuffer volumetricclouds.cpp");
			return false;
		}
	}
	...
	std::string vsData;
	{
		...
		if (FAILED(result))
		{
			LogError("Failed to create vertex shader, volumetric clouds");
			return false;
		}
		vsFile.close();
		...
		std::string csData;
		std::ifstream csFile;
		csFile.open(shaderPath + "WorleyNoiseGenerator_CS.cso", std::ios::binary);
		...
		if (FAILED(result))
		{
			LogError("Failed to create compute shader, volumetric clouds");
			return false;
		}
		csFile.close();
		std::ifstream psFile;
		psFile.open(shaderPath + "VolumetricClouds_PS.cso", std::ios::binary);

		std::string psData = { std::istreambuf_iterator<char>(psFile), std::istreambuf_iterator<char>() };
		result = device->CreatePixelShader(psData.data(), psData.size(), nullptr, &myPixelShader);
		if (FAILED(result))
		{
			LogError("Failed to create pixel shader, volumetric clouds");
			return false;
		}
		psFile.close();
	}
	...
	if (FAILED(result))
	{
		LogError("Failed to create input layout, volumetric clouds");
		return false;
	}
	return true;
}
